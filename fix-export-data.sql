-- Fix export data issue
-- Run this in Supabase SQL Editor

-- First, let's check what data we currently have
SELECT 'Current registrations:' as info;
SELECT 
  r.id,
  r.participant_name,
  r.participant_email,
...
-- Update existing data with custom fields
...
-- Add more test data with different field name variations
...
SELECT 
  ...
  '{
    "jabatan": "Manager",
    "instansi": "PT Test Export 1",
    "nomor_anggota": "2024000003",
    "alamat": "Jl. Test No. 1",
    "kota": "Jakarta"
  }'::jsonb
FROM events e 
...
SELECT 
  ...
  '{
    "Jabatan": "CEO",
    "Instansi": "PT Test Export 3",
    "Nomor Anggota": "2024000005",
    "Alamat": "Jl. Test No. 3",
    "Kota": "Surabaya"
  }'::jsonb
FROM events e 
...
-- Verify the data was updated and created
SELECT 'Updated and new data:' as info;
SELECT 
  r.id,
  r.participant_name,
  r.participant_email,
...
FROM registrations r
LEFT JOIN events e ON r.event_id = e.id
WHERE r.participant_email IN (
  'arts7.creative@gmail.com', 
  'atlastitried@gmail.com',
  'test.user1@example.com',
  'test.user2@example.com',
  'test.user3@example.com'
)
